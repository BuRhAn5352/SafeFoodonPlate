        ...
        messages=[
            {
                "role": "system",
                "content": (
                    "You are SafePlate, a food safety assistant. "
...
def format_dish_reply(parsed: dict, dish: str) -> str:
    ...
    labels = {"SAFE": "SAFE TO EAT", "WARN": "CHECK FIRST", "DANGER": "AVOID THIS"}
    ...
    lines.append(
        "\n\n_⚠️ SafePlate reduces risk — always confirm with the restaurant for severe allergies._"
    )
    return "\n".join(lines)
...
def format_menu_reply(items: list) -> str:
    ...
    for item in items:
        ...
        line = f"{icons.get(s, '⚠️')} *{item['dish']}*\n  _{item.get('reason', '')}_"
